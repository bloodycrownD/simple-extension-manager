...
const props = defineProps({
    extensions: { type: Array<Extension> },
    width: Number,
    height: Number
})
...
<template>
    <TransitionGroup name="list">
        ...
        <div class="box" v-for="item in props.extensions" :key="`extension:${getExtensionId(item)}`">
            <div class="extension" @click.once="$emit('itemClick', item)" :style="{ width, height }">
                <img :src="item.img" class="logo" draggable="false">
                <vscode-badge class="packNum" v-if="item?.pck?.extensionPack?.length"
                    @click.stop="() => { item.drawerState = !item.drawerState }">
                    {{ item?.pck?.extensionPack?.length }}
                </vscode-badge>
                <div class="info">
                    <h3 class="displayName">{{ item?.pck?.displayName }}</h3>
                    <p class="discription" :style="{ width }">{{ item?.pck?.description }}</p>
                    <h4 class="publisher">{{ item?.pck?.publisher }}</h4>
                </div>
            </div>
            <DirTree v-if="item.pck.extensionPack.length&&item.drawerState" :parent-id="getExtensionId(item)"
                :extensions="store.getExtensionInPack(item.pck.extensionPack)"></DirTree>
        </div>
