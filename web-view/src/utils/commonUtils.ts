...
export class ExtensionPackage {
    ...
    constructor(displayName?: string, discription?: string, name?: string, extensionPack?: string[]) {
        !discription || (this.description = discription);
        !displayName || (this.displayName = displayName);
        !name || (this.name = name);
        !extensionPack || (this.extensionPack = extensionPack);        
    }
}
...
export class Extension {
    ...
    public static copy(item:Extension):Extension{
        return new Extension(new ExtensionPackage(
            item.pck.displayName,
            item.pck.description,
            item.pck.name,
            item.pck.extensionPack
        ),item.img);
    }
}
