...
for (let index = 0; index < 10; index++) {
    extensions.extensionArray.push({
        pck: {
...
<template>
    <div class="outer">
        <div class="top">
            <p class="title">custom extension pack management</p>
            <vscode-button class="create">create</vscode-button>
        </div>
        <div class="bottom">
            <div class="extensionPack" v-for="item,key in extensions.extensionArray" :key="key">
                <img :src="item.img" class="icon" draggable="false">
                <div class="info">
                    <p class="disciption">{{ item.pck.description }}</p>
                    <p class="displayName">{{ item.pck.displayName }}</p>
                    <p class="publisher">{{ item.pck.publisher }}</p>
                </div>
            </div>
        </div>
    </div>
...
@extensionPackWidth:@bottomWidth * 0.9;
@extensionPackHeight:10vh;

@topHeight:30vh;
@topWidth:@bottomWidth;

.alignMiddle(@height,@width){
    width: @width;
    height: @height;
    margin: 0 auto;
}
...
.outer{
    ...
    height: @outerHeight;
    .top{
        .alignMiddle(@topHeight, @topWidth)
    }
    .bottom{
        overflow: auto;
        .alignMiddle(@bottomHeight,@bottomWidth);
        .extensionPack{
            display: flex;
