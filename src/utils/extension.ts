...
export default class Extension {
    ...
    private _img: Buffer = Buffer.from(defaultImg, "base64");
    ...
    public static readFromFile(path: string): Extension | undefined {
        ...
        if (tmpPck) {
            ...
            tmpExtension.originExtensionDirPath = path;
            return tmpExtension;
        }
    ...
    public createExtension(path: string) {
        ...
        mkdir(dirPath, err => {
            if (!err) {
                ...
                writeFile(join(dirPath, "logo.png"), this.img, err => {
                    if (err) {
                        showErrMsg(`create failed:${err?.message}`);
                    }
                });
    ...
    public get img(): Buffer {
        return this._img;
    }
    public set img(value: Buffer) {
        this._img = value;
    }
