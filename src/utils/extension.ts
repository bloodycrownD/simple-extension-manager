...
export default class Extension {
    ...
    private initalExtensionDirPath:string = '';

    constructor(pck: ExtensionPackage) {
        this.pck = pck;
    }
    ...
    public static readFromFile(path:string):Extension|undefined{
        ...
        if (tmpPck) {
            const tmpExtension = new Extension(tmpPck);
            tmpExtension.initalExtensionDirPath = path;
            return tmpExtension;
        }
        return undefined;
    }
    ...
    public createExtension(path:string){
        ...
        mkdir(dirPath,err=>{
            if(!err){
                writeFile(join(dirPath,"package.json"),this.pck.toString(),"utf8",err=>{console.log(err);});
                writeFile(join(dirPath,"logo.png"),Buffer.from(this.img,"base64"),err=>{console.log(err);});
                writeFile(join(dirPath,"README.md"),this.readme,"utf8",err=>{console.log(err);});
            }
        });
        const extensionRegisterInfos =  <RegisterInfo[]>JSON.parse(readFileSync(join(path,"extensions.json"),"utf-8"));
        extensionRegisterInfos.push(new RegisterInfo(this.pck,path));
        writeFile(join(path,"extensions.json"),JSON.stringify(extensionRegisterInfos),"utf8",err=>{console.log(err);});
    }
...
export class RegisterInfo{
    ...
    private relativeLocation:string;
